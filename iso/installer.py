...
"""
        ...
        hub_overrides_content = """\
# Auto-generated by Sovran Hub — do not edit manually.
# Human overrides belong in /etc/nixos/custom.nix (takes precedence).
{ config, lib, ... }:
{
  sovran_systemsOS.features.haven           = lib.mkDefault false;
  sovran_systemsOS.features.bip110          = lib.mkDefault false;
  sovran_systemsOS.features.mempool         = lib.mkDefault false;
  sovran_systemsOS.features.element-calling = lib.mkDefault false;
  sovran_systemsOS.features.bitcoin-core    = lib.mkDefault false;
  sovran_systemsOS.features.rdp             = lib.mkDefault false;

  sovran_systemsOS.services.synapse         = lib.mkDefault true;
  sovran_systemsOS.services.bitcoin         = lib.mkDefault true;
  sovran_systemsOS.services.vaultwarden     = lib.mkDefault true;
  sovran_systemsOS.services.wordpress       = lib.mkDefault true;
  sovran_systemsOS.services.nextcloud       = lib.mkDefault true;

  sovran_systemsOS.nostr_npub               = lib.mkDefault "";
}
"""
        proc = subprocess.run(
            ["sudo", "tee", "/mnt/etc/nixos/hub-overrides.nix"],
            input=hub_overrides_content, text=True, capture_output=True
        )
        log(proc.stdout)
        if proc.returncode != 0:
            raise RuntimeError(f"Failed to write hub-overrides.nix: {proc.stderr}")
    ...
    def do_install(self, buf):
        ...
        for f in ["/mnt/etc/nixos/role-state.nix", "/mnt/etc/nixos/custom.nix", "/mnt/etc/nixos/hub-overrides.nix"]:
