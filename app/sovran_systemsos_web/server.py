...
HUB_OVERRIDES_PATH = "/etc/nixos/hub-overrides.nix"
...
# ── Feature/service toggle constants ─────────────────────────────
...
DEFAULT_SERVICES: dict[str, bool] = {
    "synapse": True,
    "bitcoin": True,
    "vaultwarden": True,
    "wordpress": True,
...
class FeatureToggleRequest(BaseModel):
    type: str      # "feature" or "service"
    name: str
    enabled: bool
    npub: str = ""  # only for Haven


class DomainSetRequest(BaseModel):
    name: str
    domain: str
    ddns_curl: str = ""  # optional Njal.la DDNS curl command
...
def _write_rebuild_status(status: str):
    try:
        with open(REBUILD_STATUS, "w") as f:
            f.write(status)
    except OSError:
        pass
...
_HUB_OVERRIDES_TEMPLATE = """\
# Auto-generated by Sovran Hub — do not edit manually.
# Human overrides belong in /etc/nixos/custom.nix (takes precedence).
{{ config, lib, ... }}:
{{
  sovran_systemsOS.features.haven           = lib.mkDefault {haven};
  sovran_systemsOS.features.bip110          = lib.mkDefault {bip110};
  sovran_systemsOS.features.mempool         = lib.mkDefault {mempool};
  sovran_systemsOS.features.element-calling = lib.mkDefault {element_calling};
  sovran_systemsOS.features.bitcoin-core    = lib.mkDefault {bitcoin_core};
  sovran_systemsOS.features.rdp             = lib.mkDefault {rdp};

  sovran_systemsOS.services.synapse         = lib.mkDefault {synapse};
  sovran_systemsOS.services.bitcoin         = lib.mkDefault {bitcoin};
  sovran_systemsOS.services.vaultwarden     = lib.mkDefault {vaultwarden};
  sovran_systemsOS.services.wordpress       = lib.mkDefault {wordpress};
  sovran_systemsOS.services.nextcloud       = lib.mkDefault {nextcloud};

  sovran_systemsOS.nostr_npub               = lib.mkDefault "{nostr_npub}";
}}
"""


def _bool_nix(val: bool) -> str:
    return "true" if val else "false"


def _read_hub_overrides() -> dict:
    """Parse hub-overrides.nix and return current feature/service/npub state."""
    features = dict(DEFAULT_FEATURES)
    services = dict(DEFAULT_SERVICES)
    nostr_npub = ""
    try:
        with open(HUB_OVERRIDES_PATH, "r") as f:
            content = f.read()
        for m in re.finditer(
            r'sovran_systemsOS\.features\.([a-zA-Z0-9_-]+)\s*=\s*lib\.mkDefault\s+(true|false)\s*;',
            content,
        ):
            name, val = m.group(1), m.group(2)
            if name in features:
                features[name] = val == "true"
        for m in re.finditer(
            r'sovran_systemsOS\.services\.([a-zA-Z0-9_]+)\s*=\s*lib\.mkDefault\s+(true|false)\s*;',
            content,
        ):
            name, val = m.group(1), m.group(2)
            if name in services:
                services[name] = val == "true"
        npub_m = re.search(
            r'sovran_systemsOS\.nostr_npub\s*=\s*lib\.mkDefault\s+"([^"]*)"',
            content,
        )
        if npub_m:
            nostr_npub = npub_m.group(1)
    except FileNotFoundError:
        pass
    return {"features": features, "services": services, "nostr_npub": nostr_npub}


def _write_hub_overrides(features: dict, services: dict, nostr_npub: str) -> None:
    """Regenerate hub-overrides.nix from the given state dicts."""
    content = _HUB_OVERRIDES_TEMPLATE.format(
        haven=_bool_nix(features.get("haven", False)),
        bip110=_bool_nix(features.get("bip110", False)),
        mempool=_bool_nix(features.get("mempool", False)),
        element_calling=_bool_nix(features.get("element-calling", False)),
        bitcoin_core=_bool_nix(features.get("bitcoin-core", False)),
        rdp=_bool_nix(features.get("rdp", False)),
        synapse=_bool_nix(services.get("synapse", True)),
        bitcoin=_bool_nix(services.get("bitcoin", True)),
        vaultwarden=_bool_nix(services.get("vaultwarden", True)),
        wordpress=_bool_nix(services.get("wordpress", True)),
        nextcloud=_bool_nix(services.get("nextcloud", True)),
        nostr_npub=nostr_npub.replace('"', ""),
    )
    os.makedirs(os.path.dirname(HUB_OVERRIDES_PATH), exist_ok=True)
    with open(HUB_OVERRIDES_PATH, "w") as f:
        f.write(content)


def _bootstrap_hub_overrides() -> None:
    """Create hub-overrides.nix with defaults if it doesn't exist yet."""
    if not os.path.exists(HUB_OVERRIDES_PATH):
        try:
            _write_hub_overrides(DEFAULT_FEATURES, DEFAULT_SERVICES, "")
        except OSError:
            pass
...
async def api_features():
    """Return current hub-managed feature/service toggle states."""
    loop = asyncio.get_event_loop()
    overrides = await loop.run_in_executor(None, _read_hub_overrides)
    cfg = load_config()
    # Also include compiled (currently-built) state from config.json
    overrides["compiled_feature_state"] = cfg.get("feature_state", {})
    overrides["compiled_service_state"] = cfg.get("service_state", {})
    return overrides


@app.post("/api/features/toggle")
async def api_features_toggle(req: FeatureToggleRequest):
    """Update hub-overrides.nix and trigger a nixos-rebuild switch."""
    loop = asyncio.get_event_loop()
    overrides = await loop.run_in_executor(None, _read_hub_overrides)

    if req.type == "feature":
        if req.name not in DEFAULT_FEATURES:
            raise HTTPException(status_code=400, detail=f"Unknown feature: {req.name}")
        overrides["features"][req.name] = req.enabled
        if req.name == "haven" and req.npub:
            overrides["nostr_npub"] = req.npub
    elif req.type == "service":
        if req.name not in DEFAULT_SERVICES:
            raise HTTPException(status_code=400, detail=f"Unknown service: {req.name}")
        overrides["services"][req.name] = req.enabled
    else:
        raise HTTPException(status_code=400, detail="type must be 'feature' or 'service'")

    await loop.run_in_executor(
        None,
        lambda: _write_hub_overrides(
            overrides["features"],
            overrides["services"],
            overrides.get("nostr_npub", ""),
        ),
    )

    # Check if rebuild is already running
    rebuild_status = await loop.run_in_executor(None, _read_rebuild_status)
    if rebuild_status == "RUNNING":
        return {"ok": True, "status": "rebuild_already_running"}

    _write_rebuild_status("RUNNING")
    try:
        with open(REBUILD_LOG, "w") as f:
            f.write("")
    except OSError:
        pass
    ...
    return {"ok": True, "status": "rebuild_started"}


@app.get("/api/rebuild/status")
async def api_rebuild_status(offset: int = 0):
    """Poll endpoint for feature-rebuild progress."""
    loop = asyncio.get_event_loop()
...
async def api_domains():
    """Return all configured domain values from /var/lib/domains/."""
    domains: dict[str, str] = {}
    try:
        for name in os.listdir("/var/lib/domains"):
            if name.startswith("."):
                continue
            path = f"/var/lib/domains/{name}"
...
async def api_domains_set(req: DomainSetRequest):
    ...
    if req.ddns_curl:
        ...
        os.makedirs("/var/lib/njalla", exist_ok=True)
        with open("/var/lib/njalla/njalla.sh", "a") as f:
            f.write(f"\n{curl_line}\n")
...
async def _startup():
    """On server start: seed internal IP file and bootstrap hub-overrides.nix."""
    loop = asyncio.get_event_loop()
    ip = await loop.run_in_executor(None, _get_internal_ip)
    _save_internal_ip(ip)
    await loop.run_in_executor(None, _bootstrap_hub_overrides)
